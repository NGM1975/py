...
def eliminar():
    miConexión=sqlite3.connect("BDUsuarios")
    miCursor=miConexión.cursor()
    #print("DELETE FROM USUARIOS WHERE nombre = " + varNombre.get())  
    miCursor.execute("DELETE FROM USUARIOS WHERE id = " + varId.get())
...
raíz.title("Interfase Final de CRUD sin validaciones")
...
txtComentarios.config(yscrollcommand=scrollVert.set, fg='black',bg='pink')
